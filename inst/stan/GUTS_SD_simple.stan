...
https://github.com/virgile-baudrot/gutsRstan */
  ...
  functions {
    ...
    // matrix solve_TKTD_varSD(array[] real y0, real t0, array[] real ts, array[] real theta, data array[] real tconc, data array[] real conc, data real relTol, data real absTol, int maxSteps){
      matrix solve_TKTD_varSD(vector y0, real t0, array[] real ts, array[] real theta, data vector tconc, data vector conc, data real relTol, data real absTol, int maxSteps){
    ...
    transformed parameters{
      ...
      for(gr in 1:nGroup){
        ...
        /* initial time must be less than t0 = 0, so we use a very small close small number 1e-9 to at at time tNsurv and tconc */
          y_hat[idS_lw[gr]:idS_up[gr],1:2] = solve_TKTD_varSD(y0, 0, tNsurv_ode[idS_lw[gr]:idS_up[gr]], param, to_vector(tconc_ode[idC_lw[gr]:idC_up[gr]]), to_vector(conc[idC_lw[gr]:idC_up[gr]]), relTol, absTol, maxSteps);
